(function() {
	...
	ScheduleTemplate.prototype.openModal = function(target) {
		...
		////
		// self.modal.getElementsByClassName('cd-schedule-modal__event-info')[0].innerHTML = self.getEventContent('<div class=cd-schedule-modal__event-info><div>Lorem</div></div>');
		...
		if( mq == 'mobile' ) {
			self.modal.setAttribute('style', 'position: absolute; height: 50%');
	...
	ScheduleTemplate.prototype.loadEventContent = function(content) {
		...
		httpRequest.open('GET', 'schedule-template-master/' + content+'.html');
    httpRequest.send();
	};
